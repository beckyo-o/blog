...
export default async function EntryDetailPage({
  ...
  const entry = entries.find(
    (entry: { id: string }) => entry.id === params.entryId
  ...
  const nextEntryExists = entries.some(
    (entry: { id: string }) => entry.id === nextEntry
  );
  const prevEntryExists = entries.some(
    (entry: { id: string }) => entry.id === prevEntry
  );
        ...
        <div className="">
          ...
          <div className="grid grid-cols-1 gap-4 place-items-center h-56">
            {entry.images.map((image: string) => (
              <img src={image} alt="" className="py-10 h-auto max-w-full" />
            ))}
          </div>
        </div>
      </div>
